...
/** Resolve the same explicitly provisioned Chromium for every server launcher. */
export function resolveBrowserExecutablePath(
  ...
  if (!executablePath) {
    ...
    // Development/test may use the browser downloaded by the automation library.
    return undefined
  }
  ...
  if (!isAbsolute(executablePath)) throw new Error(invalidPath)
  try {
    const executable = statSync(executablePath)
    if (!executable.isFile() || executable.size === 0) throw new Error(invalidPath)
